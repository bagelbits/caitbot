...
const configureServer = () => {
  /*
   * commonWebpackConfig()'s result (even after toWebpackConfig()) is a mutable
   * global, so mutating it here (e.g. the entry value) would also change the
   * client config. Getting a fresh call result avoids that.
   */
  ...
  /*
   * Remove the mini-css-extract-plugin from the style loaders (the client
   * build handles exporting CSS) and swap file-loader for null-loader.
   */
  ...
  /*
   * Configure loader rules for SSR: remove the mini-css-extract-plugin from
   * the style loaders (the client build handles exporting CSS) and swap
   * file-loader for null-loader.
   */
